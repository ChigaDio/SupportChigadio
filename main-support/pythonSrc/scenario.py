...
def generate_all_event_bin():
    ...
    for event in events:
        ...
        if not isinstance(subgroups, dict):
            logger.error(f"Invalid subgroups in event {event.get('id', 'unknown')}: {subgroups}")
            continue
        logger.debug(f"Generating bin for event {event.get('id', 'unknown')}: {len(subgroups)} subgroups")
        section.extend(struct.pack('i', len(subgroups)))
