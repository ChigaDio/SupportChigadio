 ...
 * 戻り値は出現順(=行番号順)の配列。
 */
function findGroupHeaders(docText) {
 ...
 */
function computeNextGroupHeader(docText, cursorLine) {
 ...
 */
function computeNextSubgroupHeader(docText, cursorLine) {
  const headers = findGroupHeaders(docText);
  let currentSubId = null;
 ...
 */
function coerceValueTokens(valueTokens, fieldType, fieldOptions, lineText, subFields) {
...
function compileDocument(text, roleSchemas, existingRoles = []) {
...
function decompileRoles(roles, roleSchemas) {
  const lines = [];
...
function lintDocument(text, roleSchemas) {
...
function getCompletionsAt(text, cursorLine, cursorCh, roleNames, roleSchemas) {
  const lines = text.split('\n');
